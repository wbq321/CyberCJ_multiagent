...
def set_profile():
    """Allow users to set their profile (placeholder for now)"""
    data = request.get_json()
    session_id = data.get('session_id', 'default')
    user_profile = data.get('user_profile', 'general')

    return jsonify({
        "message": f"Profile set to {user_profile}",
        "session_id": session_id,
...
@app.route('/set_profile', methods=['OPTIONS'])
@app.route('/new_topic', methods=['OPTIONS'])
...
def serve_interface():
    """Serve the improved tutor HTML interface"""
    try:
        with open('improved_tutor.html', 'r', encoding='utf-8') as f:
            html_content = f.read()
        return html_content
    except FileNotFoundError:
        return """
        <h1>CyberJustice Tutor API</h1>
        <p>Server is running! Use the /ask endpoint to interact with the tutor.</p>
        <p>Or create an improved_tutor.html file to serve the interface.</p>
        """

# --- Serve Multi-Agent Interface ---
@app.route('/multi', methods=['GET'])
def serve_multi_interface():
    """Serve the multi-agent chat HTML interface"""
    try:
        ...
        return """
        <h1>Multi-Agent Interface Not Found</h1>
        <p>Create a multi_agent_chat.html file to serve the multi-agent interface.</p>
